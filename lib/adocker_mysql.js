...
function adockerMysql (name, options = {}) {
  ...
  return Object.assign(bundle, {
    cli () {
      ...
      cli.call = (argv = process.argv) => co(function * () {
        let [ name, ...args ] = argv.slice(2)
        if (name) {
          let cmd = cli[ name ]
          if (!cmd) {
            throw new Error(`[adocker-mysql] Command ${name} is not found. (Available: ${JSON.stringify(Object.keys(cli))}`)
          }
          return cmd(args)
        } else {
          console.log('Available commands:')
          console.log('')
          for (let name of Object.keys(cli)) {
            console.log('\t', name)
          }
          console.log('')
        }
